    ...
    add_banner = async(req,res) => {
       ...
       form.parse(req, async(err, field, files) => {
        ...
        try {
            ...
            const result = await cloudinary.uploader.upload(mainban[0].filepath, { folder: 'banners' });
    ...
    deleteBanners = async (productId) => {
        try {
            ...
            const bannerDeletionPromises = banners.map(async (banner) => {
                ...
                return cloudinary.uploader.destroy(`banners/${bannerPublicId}`);
            });
